...
void send_int(const char* server_ip, int server_port, int number) {
    // 创建 TCP 客户端套接字
    ...
    // 发送整数数据
    int network_number = htonl(number);
    send(client_socket, &network_number, sizeof(network_number), 0);
    // 关闭套接字
    close(client_socket);
}


void print_topk_and_send(float* data, int total_num, int topk, const char* server_ip,int server_port)
{
    assert(total_num >= topk);
    cls_score* cls_scores = (cls_score*)malloc(total_num * sizeof(cls_score));
    for (int i = 0; i < total_num; i++)
    ...
    send_int(server_ip,server_port,cls_scores[0].id);


    free(cls_scores);
}

int print_topk_and_iftrue(float* data, int total_num, int topk, int truedata){
    assert(total_num >= topk);
    cls_score* cls_scores = (cls_score*)malloc(total_num * sizeof(cls_score));
    for (int i = 0; i < total_num; i++)
    {
...
int result = cls_scores[0].id==truedata;
