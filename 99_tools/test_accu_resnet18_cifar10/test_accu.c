                     ...
                     int loop_count, int num_thread)
{
    ...
    tensor_t input_tensor = get_graph_input_tensor(graph, 0, 0);//得到这个tensor的索引
    ...
    if (set_tensor_shape(input_tensor, dims, 4) < 0)//将这个tensor的形状设置为dims
    {
...
FILE *file;
    ...
    // 打开 labels.txt 文件                             ale;
    file = fopen("/mnt/sdb3/labels.txt", "r");
    if (file == NULL) {
        printf("无法打开文件！\n");
        return 1;
    }
int correctNum = 0;
int totalNum = 0;
for(int photonum=0;fscanf(file, "%d", &data) != EOF & photonum<TESTNUM;photonum++){


    char tempimagefile[40] = "/mnt/sdb3/img/test_";
    char numStr[20];
    sprintf(numStr,"%d",photonum);
printf("%s\n",numStr);
  
    strcat(tempimagefile,numStr);
printf("%s\n",tempimagefile);
    char endfile[20] = ".jpg";
    strcat(tempimagefile,endfile);
    printf("%s\n",tempimagefile);

image_file = tempimagefile;

    /* prepare process input data, set the data mem to input tensor */
    // 准备处理输入数据，将数据内存设置为输入张量
    float input_scale = 0.f;
    int input_zero_point = 0;
    get_tensor_quant_param(input_tensor, &input_scale, &input_zero_point, 1);
    get_input_int8_data(image_file, input_data, img_h, img_w, mean, scale, input_scale);//将图片数据转换为int8
    if (set_tensor_buffer(input_tensor, input_data, img_size) < 0)//将input_tensor的buffer设置为input_data
    {
   ...
   int ifcorrect =  print_topk_and_iftrue(output_data, output_size, 5,data);
   printf("correct:%d\n",ifcorrect);
    correctNum += ifcorrect;
    totalNum += 1;

}

printf("correct:%d,total:%d,accuracy:%f\n",correctNum,totalNum,(double)correctNum/totalNum);
